...
export default function Request({data}) {
    ...
    function btnCloseClick() {
        ...
        closeRequest(data.id).then(result => {
            alert("Pedido fechado com sucesso. Aguarde alguns minutos")
    ...
    function btnHelpClick(){
        const donationInBnb = prompt("O quanto deseja ajudar (em BNB) ?")
        donate(data.id, donationInBnb).then(result => {
            alert("Ajuda feita com sucesso. Aguarde alguns minutos")
            window.location.reload()
        }).catch(err => {
            console.error(err);
            alert(err.message)
